...
public class PerlinNoise extends Noise {
    ...
    private static int[] p;                                                    // floatd permutation to avoid overflow
...
//            93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180};
 ...
 /*   private float getPoint(float x, float y) {
        ...
        float xfade = fade(x);
        float yfade = fade(y);
        int NW = (int) (Math.random() * 4);
        int NE = (int) (Math.random() * 4);
        int SW = (int) (Math.random() * 4);
        int SE = (int) (Math.random() * 4);
        float gNW = grad(aa % 4, x, y);
        float gNE = grad(ab % 4, x, y - 1);
        float gSW = grad(ba % 4, x - 1, y);
