...
public class PerlinNoise extends Noise {
    ...
    public double[][] generateMap() {
        double[][] noiseMap = new double[xres * 10][yres * 10];
    ...
    private double getPoint(double x, double y) {
        ...
        double gSE = grad(SE % 4, x, y);
        double top = lerp(xfade, gNW, gNE);
        double bot = lerp(xfade, gSW, gSE);
        return lerp(yfade, top, bot);
    }

    private double grad(int dir, double x, double y) {
